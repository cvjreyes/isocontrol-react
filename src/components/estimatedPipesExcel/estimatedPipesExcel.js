...
class EstimatedPipesExcel extends React.Component {
  ...
  async filter(column, value) {
    ...
    for (let i = 0; i < auxDisplayData.length; i++) {
      ...
      for (
        ...
        console.log(
          "Includes " +
            fil +
            ": " +
            !auxColumn.toString().includes(this.state.filterData[column])
        );
        if (
          this.state.filterData[column] &&
          !auxColumn.toString().includes(this.state.filterData[column])
      ...
      console.log(
        "Includes: " +
          !auxDisplayData[i]["Tag"]
            .toString()
            .includes(this.state.filterData[1])
      );
      if (exists) {
        resultData.push(auxDisplayData[i]);
  ...
  async submitChanges() {
    ...
    Object.entries(this.state.new_data).map(([key, value]) =>
      new_rows.push(value)
    ); //Por cada linea de la tabla nueva o modificada
  ...
  handleChange = async (changes, source) => {
    //Este metodo se llama cada vez de que se modifica un campo en la tabla
    if (source !== "loadData") {
      //Si el cambio no se debe a la carga de datos (es decir cuando se ha modificado algo)
      ...
      for (let i = 0; i < changes.length; i += 4) {
        ...
        if (changes[i][1] === "Owner IFC") {
          ...
          owners.push([
            ...
            changes[0][3],
          ]); //Guardamos el owner en un array para guardarlo en la bd cuando hagamos el submit
          this.setState({ owners: owners });
        } else if (changes[i][1] === "Owner IsoTracker") {
          //Si el cambio se ha producido en la columna owner
          let owners = this.state.owners;
          owners.push([
            "ISO",
            this.state.displayData[changes[i][0]].Tag,
          ...
          //Si el cambio se ha producido en cualquier otra columna
          let row_id = changes[i][0];
          if (
            this.state.displayData[row_id].Status === "MODELLED*" ||
            this.state.displayData[row_id].Status === "MODELLED"
          ) {
            //Si la linea estaba modelada
            ...
            if (changes[i][1] === "Line reference") {
              //Si el cambio ha sido en el campo de la line reference
              const options = {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (
              ...
              //Si la linea tiene todos los campos llenos (excepto el owner que no es olbigatorio)
              //Generamos el tag
              let tag_order = process.env.REACT_APP_TAG_ORDER.split(/[ -]+/);
              let tag = "";
              for (
            ...
            if (
              ...
              //Si el tag generado ya existe quiere decir que la linea esta repetida
              ...
              //Si no le colocamos el status ESTIMATED y obtenemos el TL
        ...
        <div id="hot-app">
          ...
          <HotTable
            ...
            columns={[
              ...
              { data: "Tag", type: "text", readOnly: true },
              {
              ...
              { data: "Fluid", type: "text", readOnly: true },
              { data: "Seq", type: "text", readOnly: true },
              { data: "Spec", type: "text", readOnly: true },
              ...
              { data: "Insulation", type: "text", readOnly: true },
              {
                data: "Train",
                type: "dropdown",
