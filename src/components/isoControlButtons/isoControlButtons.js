...
const IsoControlButtons = () => {
  ...
  // Botones
  const [feedBtn, setFeedBtn] = useState(
    <div className="container__buttons__iso__ctrl">
      <a
        href={"/" + process.env.REACT_APP_PROJECT + "/piping"}
        onClick={() => setFeedEnter(true)}
        className="button__iso__ctrl"
      >
        <center className="label__iso__ctrl">FEED</center>
        <center className="description__iso__ctrl">
          (Front end Engineering Design)
        </center>
        <p
          className={`percentage__iso__ctrl ${feedDecimals ? "" : "move__num"}`}
        >
          {feedProgress}%
        </p>
      </a>
  ...
  const [ifdBtn, setIfdBtn] = useState(
    <div className="container__buttons__iso__ctrl">
      <a
        href={"/" + process.env.REACT_APP_PROJECT + "/piping"}
        onClick={() => setFeedEnter(false)}
        className="button__iso__ctrl"
      >
        <center className="label__iso__ctrl">IFD</center>
        <center className="description__iso__ctrl">(Issue for Design)</center>
        <p
          className={`percentage__iso__ctrl ${ifdDecimals ? "" : "move__num"}`}
        >
          {ifdProgress}%
        </p>
      </a>
  ...
  const [ifcBtn, setIfcBtn] = useState(
    <div className="container__buttons__iso__ctrl">
      <a
        href={"/" + process.env.REACT_APP_PROJECT + "/isotracker"}
        className="button__iso__ctrl"
      >
        <center className="label__iso__ctrl">IFC</center>
        <center className="description__iso__ctrl">
          (Issue for Construction)
        </center>
        <p
          className={`percentage__iso__ctrl ${ifcDecimals ? "" : "move__num"}`}
        >
          {ifcProgress}%
        </p>
      </a>
  ...
  useEffect(async () => {
    ...
    fetch(
      ...
      .then((json) => {
        ...
        setTotalProgress(
          (feedProgress * 0.1 + ifdProgress * 0.4 + ifcProgress * 0.5).toFixed(
            2
          )
        );
        ...
        setFeedBtn(
          <div className="container__buttons__iso__ctrl">
            ...
            >
              ...
              <p
                className={`percentage__iso__ctrl ${
                  feedDecimals ? "" : "move__num"
                }`}
              >
                {feedProgress}%
              </p>
      ...
      <div className="container__progress__iso__ctrl">
        ...
        <div className="container__progress">
          <div className="progress__images">
            <ProgressBar>
              <p className="percentage__progress">{totalProgress}%</p>
            </ProgressBar>
          </div>
        </div>
      </div>
