...
class FeedPipesExcel extends React.Component {
  ...
  async submitChanges() {
    ...
    let new_rows = [...this.state.newData];
    // Object.entries(this.state.newData).map(([key, value]) =>
    //   new_rows.push(value)
    // );
    debugger;
    console.log(3, "new_rows: ", new_rows);
    for (let i = 0; i < new_rows.length; i++) {
      if (
        ...
        console.log(
          4,
          "New Rows: " + new_rows.forEach((x, i) => console.log(i, x))
        );
    ...
    await fetch(
      ...
      .then(async (json) => {
        ...
        this.setState({ newData: [], warning: false, empty: false });
      });
      ...
      for (let i = 0; i < changes.length; i += 4) {
        if (changes[i][1] === "Owner IFC") {
          ...
          if (!row["Line reference"] && row.id) {
            let newData = this.state.newData;
            newData.push({ "Line reference": "deleted", id: row.id });
